...
```python
from galaxeo.arm import Arm, BusTransport, Sphere
from galaxeo.bus import open_bus

arm = Arm(BusTransport(open_bus("can0"), tx=True), armed=True)
plan = arm.plan([0.35, 0.0, 0.10], obstacles=[Sphere((0.3, 0.1, 0.05), 0.04)])
if plan:                                  # else plan.reason: outside volume | ik | jump | collision
    result = arm.move(plan.joints, speed=20.0)   # reached | backoff | hold | rejected, + reason
arm.stop()                                # any thread: hold the measured pose, disarm
```

Fixed, not configured: Galaxea A1X only; base_link at the origin, x forward
over the table, z up; the table top is the plane z = 0. The tool frame is a
fixed transform from `arm_link6` (`Tool(pos, quat)`, default the gripper's
point between the finger plates). Joints are radians, as on the wire.

* **Model** - `galaxeo/arm/assets/a1x.xml`, generated by `sim/urdf2mjcf.py`
  from the URDF; joint limits come from it.
* **Kinematics** - FK and IK of the tool frame in MuJoCo. IK reports its
  residual, keeps to the limits, and puts the point before the orientation.
* **Collision** - self, table and caller boxes/spheres; poses and whole paths.
* **Reach box** - `arm.reach`, x 0.20..0.50, y ±0.30, z 0.05..0.30 m for the
  gripper: every corner of a 4x4x3 grid solves within 3 mm and is clear.
  Cached in `assets/reach.json`, recomputed when the model or tool changes.
* **Move** - from the measured pose, smoothstep ramp at the given deg/s,
  streamed at 200 Hz on CAN (any rate). Refuses to start on missing, stale or
  frozen feedback and on a jump over 45 deg; stops streaming on feedback older
  than 150 ms. The impact guard trips on lag (command moving, joint still) or
  effort; the arm then holds the measured pose and backs off at 8 deg/s to a
  pose it was at 0.4 s earlier, at least 3 deg away. A second trip only
  holds. Never sends FF 2. Details in `galaxeo/arm/motion.py`.
* **Transports** - `BusTransport` (any `galaxeo.bus`), `SimTransport`
  (MuJoCo), `galaxeo.arm.fake.FakeA1XBus` for tests.
