...
class Student:
    def __init__(self, obs_dim, act_dim=5, width=32, device="cpu"):
        self.net = StudentNet(obs_dim, act_dim, width).to(device)
        self.device = device
    ...
    def save(self, path):
        torch.save(dict(net=self.net.state_dict(), obs_dim=self.obs_dim, act_dim=self.act_dim, width=self.width,
                        obs_mean=self.obs_mean, obs_std=self.obs_std), path)

    @classmethod
    def load(cls, path, device="cpu"):
        s = torch.load(path, map_location=device, weights_only=False)
        st = cls(s["obs_dim"], s["act_dim"], s["width"], device)
...
def train(a):
    ...
    st = Student.load(a.init, dev) if a.init else Student(obs.shape[1], acts.shape[1], a.width, dev)
    if not a.init:
