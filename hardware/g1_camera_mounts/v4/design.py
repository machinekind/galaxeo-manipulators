"""Fixed 75-degree G1 camera brackets, revision 4. All dimensions in mm.
...
def reference(d):
 # Keep a 100 mm nominal working distance and aim at X=75, Z=0.
 axis=rotation()[:,0]
 return np.array([75.,0.,(d-60)/2])-(WORKING_DISTANCE+14.6)*axis
...
def clamp(d,upper):
 r=(d+.6)/2;R=r+5
 p=cq.Workplane('YZ',origin=(CLAMP_X0,0,0)).circle(R).circle(r).extrude(18)
 half=box(CLAMP_X0-1,CLAMP_X1+1,-70,70,.75,60) if upper else box(CLAMP_X0-1,CLAMP_X1+1,-70,70,-60,-.75)
 p=p.intersect(half)
 for side in (-1,1):
  y=side*(R+5)
  ear=box(CLAMP_X0,CLAMP_X1,y-7,y+7,.75,9) if upper else box(CLAMP_X0,CLAMP_X1,y-7,y+7,-9,-.75)
  p=p.union(ear.edges('|Z').fillet(2)).cut(cylinder_z(CLAMP_X,y,-12,12,2.25))
  if not upper:
   pocket=cq.Workplane('XY',origin=(CLAMP_X,y,-9.1)).polygon(6,7.25/math.cos(math.pi/6)).extrude(3.4)
   p=p.cut(pocket)
 return p.clean()
...
def upper(kind,d):
 ...
 profile=[(CLAMP_X0,root_z),(CLAMP_X1,root_z),(CLAMP_X1,38),(ends[0][0],ends[0][2]),(ends[1][0],ends[1][2])]
 for side in (-1,1):
...
def parts():
 out={}
 for d in DIMS:
  for kind in ('board','webcam'):out[f'{kind}_fixed{ANGLE}_upper_{d}mm']=(upper(kind,d),'side')
