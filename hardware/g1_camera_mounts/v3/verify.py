...
def main():
 ...
 for d in DIMS:
  ...
  for side in (-1,1):
   y=side*((d+.6)/2+10)
   nut=cq.Workplane('XY',origin=(-45,y,-8.9)).polygon(6,7/math.cos(math.pi/6)).extrude(3.2)
  ...
  for kind in ('board','webcam'):
   ...
   for side in (-1,1):
    y=side*((d+.6)/2+10)
    bolt=cylinder_z(-45,y,-10.2,9.8,2).union(cylinder_z(-45,y,9,9.8,4.5)).union(cylinder_z(-45,y,9.8,13.8,3.5))
 ...
 'limits':'Physical fit/load and exact camera not tested. Fixed 60 degree pitch; arbitrary lens height changes framing. No cables or full-arm sweep in model.',
